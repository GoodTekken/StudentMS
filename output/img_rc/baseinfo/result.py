...
class Ui_MainWindow(QMainWindow):
    ...
    def query(self):
        ...
        for i in range(row):  # 遍历行
            for j in range(self.tbResult.columnCount()):  # 遍历列
                data = QTableWidgetItem(str(result[i][j]).encode('latin1').decode('gbk'))  # 转换后可插入表格
    ...
    def bindGrade(self):
        ...
        for i in result:  # 遍历查询结果
            self.cboxGrade.addItem(i[0].encode('latin1').decode('gbk'))  # 在下拉列表中显示年级
    ...
    def bindClass(self):
        ...
        for i in result:  # 遍历查询结果
            self.cboxClass.addItem(i[0].encode('latin1').decode('gbk'))  # 在下拉列表中显示年级
    ...
    def bindCbox(self):
        ...
        for i in result:  # 遍历查询结果
            self.cboxKinds.addItem(i[0].encode('latin1').decode('gbk'))  # 在下拉列表中显示考试类别
        self.cboxSubject.addItem("所有")  # 设置考试科目的默认首选项
        result = service.query("select subName from tb_subject")  # 从考试科目表中查询数据
        for i in result:  # 遍历查询结果
            self.cboxSubject.addItem(i[0].encode('latin1').decode('gbk'))  # 在下拉列表中显示考试科目
        result = service.query("select stuName from tb_student")  # 从学生信息表中查询数据
        for i in result:  # 遍历查询结果
            self.cboxStuName.addItem(i[0].encode('latin1').decode('gbk'))  # 在下拉列表中显示所有学生姓名
    ...
    def bindStuName(self):
        ...
        for i in result:  # 遍历查询结果
            self.cboxStuName.addItem(i[0].encode('latin1').decode('gbk'))  # 在下拉列表中显示指定班的学生姓名
    ...
    def getItem(self, item):
        if item.column()==0: # 如果单击的是第一列
            ...
            self.cboxKinds.setCurrentText(result[0][3].encode('latin1').decode('gbk'))  # 显示考试类别
            self.cboxGrade.setCurrentText(result[0][6].encode('latin1').decode('gbk'))  # 显示年级
            self.cboxClass.setCurrentText(result[0][5].encode('latin1').decode('gbk'))  # 显示班级
            self.cboxStuName.setCurrentText(result[0][2].encode('latin1').decode('gbk'))  # 显示学生姓名
            self.cboxSubject.setCurrentText(result[0][4].encode('latin1').decode('gbk'))  # 显示考试科目
            self.editResult.setText(str(result[0][7]).encode('latin1').decode('gbk'))  # 显示学生分数
